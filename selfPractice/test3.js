...
function torn(cashProduct, cashCustomer) {
    if (+cashCustomer < +cashProduct) return console.log('เงินไม่พอ');
    let cash = {
        cash_500: 0,
        cash_100: 0,
        cash_50: 0,
    ...
    if (charge >= 500) {
        cash['cash_500'] = Math.floor(charge / 500)
        charge = charge - 500 * Math.floor(charge / 500)
    }
    //* กรณีเงินทอนมากกว่าเท่ากับ 100
    if (charge >= 100) {
        cash['cash_100'] = Math.floor(charge / 100)
